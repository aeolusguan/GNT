...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        # Always expect 3 scales; enforce intermediate idx = (0, 1, 2)
        self.intermediate_layer_idx: Tuple[int, int, int] = (0, 1, 2)

        self.project = nn.Conv2d(dim_in, out_channels[-1], kernel_size=1, stride=1, padding=0)
    ...
    def forward(
        ...
        """
        B, S, N, C = aggregated_tokens_list[-1][0].shape
        gate = torch.softmax(self.gate(aggregated_tokens_list[-1][0]), dim=1)
        feat = (gate * aggregated_tokens_list[-1][0]).sum(dim=1)  # B,N,C
        ph, pw = H // self.patch_size, W // self.patch_size
        feat = self.project(feat[:, patch_start_idx:].transpose(-1, -2).view(B, C, ph, pw))  # B,C,ph,pw
        feats = aggregated_tokens_list[:-1] + [feat]  # Replace last layer's feat with gated version for projection and fusion.

        if chunk_size is None or chunk_size >= B:
            preds, conf = self._forward_impl(feats, H, W)
            return preds, conf
        preds, conf = [], []
    ...
    def _forward_impl(
        ...
        for stage_idx, take_idx in enumerate(self.intermediate_layer_idx):
            x = aggregated_tokens_list[take_idx].permute(0, 2, 3, 1)
            x = x.permute(0, 3, 1, 2)
