...
def export_blender_cache(options: BlenderExportOptions) -> dict[str, object]:
    ...
    for keyframe_idx in range(map_start_keyframe, max_keyframe + 1):
        ...
        points_cam, pixels = backproject_depth(
            depth,
            mask,
            depth_intrinsics[keyframe_idx],
            stride=options.point_stride,
        )
    ...
    np.savez_compressed(
        ...
        keyframe_intrinsics=artifacts.intrinsics.astype(np.float32),
        frame_intrinsics=artifacts.frame_intrinsics.astype(np.float32),
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Render a publication-style Blender video from GeNT artifacts.")
    parser.add_argument(
        "--recipe",
