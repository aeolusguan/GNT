...
__global__ void projection_distance_kernel(
    ...
    const float* source_intrinsics = intrinsics + source * 4;
    const float* target_intrinsics = intrinsics + target * 4;
    const float source_fx = source_intrinsics[0];
    const float source_fy = source_intrinsics[1];
    const float source_cx = source_intrinsics[2];
    const float source_cy = source_intrinsics[3];
    const float target_fx = target_intrinsics[0];
    const float target_fy = target_intrinsics[1];
    const float target_cx = target_intrinsics[2];
    const float target_cy = target_intrinsics[3];
    ...
    for (int sample = threadIdx.x; sample < total_samples; sample += blockDim.x) {
        ...
        const float X0 = (static_cast<float>(sx) - source_cx) / source_fx;
        const float Y0 = (static_cast<float>(sy) - source_cy) / source_fy;
        const float Z0 = 1.0f;
        ...
        const float px = target_fx * X1 * inv_Z1 + target_cx;
        const float py = target_fy * Y1 * inv_Z1 + target_cy;
...
torch::Tensor projection_distance_cuda(
    ...
    TORCH_CHECK(
        intrinsics.dim() == 2 && intrinsics.size(0) == poses.size(0) && intrinsics.size(1) == 4,
        "intrinsics must have shape (N, 4)");
