...
class StandardResizeStreamProcessor(StreamProcessor):
    ...
    def _compute_frame_size(self, previous_frame_size: tuple[int, int]) -> tuple[int, int]:
        h0, w0 = previous_frame_size
        scale_factor = (self.pixel_budget / (h0 * w0)) ** 0.5
        h1 = max(self.patch_size, round(h0 * scale_factor / self.patch_size) * self.patch_size)
        w1 = max(self.patch_size, round(w0 * scale_factor / self.patch_size) * self.patch_size)

        self.fac_x, self.fac_y = w0 / w1, h0 / h1
        return h1, w1

    def update_frame_size(self, previous_frame_size: tuple[int, int]):
        return self._compute_frame_size(previous_frame_size)

    def __call__(self, frame_idx: int, frame_data: VideoFrame) -> VideoFrame:
        return frame_data.resize(self._compute_frame_size(frame_data.size()))
    ...
    def recover_intrinsics(self, after_intrinsics: torch.Tensor) -> torch.Tensor:
        ...
        intrinsics[..., 2] *= self.fac_x
        intrinsics[..., 3] *= self.fac_y
        return intrinsics
...
class SLAMSystem:
    ...
    def _build_components(self):
        self.measurements = GeNTMeasurements.build(
            ...
            depth_normalization_min_cutoff=self.config.depth_normalization_min_cutoff,
            depth_normalization_quantile=self.config.depth_normalization_quantile,
        )
