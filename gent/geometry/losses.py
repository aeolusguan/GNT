...
def flow_loss(
    gt_pose: SE3,  # [B,S]
    disps: torch.Tensor,  # [B,S,H,W]
    poses_est: SE3,  # [B,E]
    disps_est: torch.Tensor,  # [B,S,H,W]
    intrinsics: torch.Tensor,  # [B,S,4]
    graph,
    valid: torch.Tensor,  # [B,S,H,W]
    flow_predictions=None,
    args=None,
):
    """Pose-induced projection loss and frontend optical-flow loss."""
    ...
    projection_valid = (val0 * val1).squeeze(dim=-1)
    projection_error = torch.where(
        projection_valid > 0.5,
        (coords1 - coords0).norm(dim=-1),
        0.0,
    )
    projection_loss = projection_error.mean()

    with torch.no_grad():
        metric_valid = projection_valid > 0.5
        metric_count = metric_valid.sum().clamp_min(1)
        metrics = {
            'f_error': projection_error.sum() / metric_count,
            '1px': ((projection_error < 1.0) & metric_valid).sum() / metric_count,
        }
    ...
    return projection_loss, front_flow_loss, metrics
