...
def save_depth_maps(depth, path, conf=None):
    depth_maps = torch.stack(depth, 0)
    ...
    if conf is not None:
        conf = torch.concat(conf, 0)
        min_conf = torch.log(conf.min())  # float(torch.quantile(out, 0.01))
        max_conf = torch.log(conf.max())  # float(torch.quantile(out, 0.99))
