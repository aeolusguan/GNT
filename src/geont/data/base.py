...
class RGBDDataset(EasyDataset):
    ...
    def __init__(self, name, datapath, n_frames=4, crop_size=[384, 512], fmin=8.0, fmax=75.0, do_aug=True):
        ...
        cache_path = osp.join(DATASET_CACHE_DIR, '{}.pickle'.format(self.name))
    ...
    def __getitem__(self, index):
        ...
        for i in inds:
            ...
            depth, depth_valid = self.__class__.depth_read(depths_list[i])
            depths.append(depth)
            if self.z_far > 0:
        ...
        depths_valid = np.stack(depths_valid).astype(np.bool)
        ...
        if self.aug is not None:
            images, poses, depths, depths_valid, intrinsics = \
                self.aug(images, poses, depths, depths_valid, intrinsics)
