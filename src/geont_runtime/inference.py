...
def build_streaming_config(
    frame_dir: str | Path,
    ckpt_path: str | Path,
    intrinsics: list[float] | tuple[float, float, float, float],
    ...
    cfg.pipeline.slam.intrinsics = list(intrinsics)
...
def run_streaming_config(cfg: DictConfig):
    ...
    from geont_runtime.streams.frame_dir_stream import FrameDirStreamList
