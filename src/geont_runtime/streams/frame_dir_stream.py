...
from geont_runtime.streams.base import ProcessedVideoStream, StreamList, VideoFrame, VideoStream
...
class FrameDirStream(VideoStream):
    ...
    def __init__(self, path: Path, seek_range: range | None = None, name: str | None = None) -> None:
        ...
        if self.end <= self.start:
            raise ValueError(
                f"Frame range [{self.start}, {self.end}) selects no frames from {path}"
            )
