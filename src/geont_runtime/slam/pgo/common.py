...
@dataclass
class Sim3PGOResult:
    """PGO output: poses is (N, 7), log_scales is (N,)."""
...
def _apply_delta(
    ...
    poses.data: (N, 7), log_s: (N,).
    pose_delta: (A, 6) = [dt(3), dphi(3)] for non-anchor nodes.
    scale_delta: (N,) dlog_s for all nodes, including the anchor.
    """
...
def _scaled_se3_residuals(
    ...
    poses.data: (N, 7), log_s: (N,), ii/jj: (E,), rel_edges.poses.data:
    (E, 7). Returns (E, 6) residuals: [translation(3), rotation_log(3)].
...
def _rotation_residuals(
    ...
    rotations.data: (N, 4), ii/jj: (E,), meas_rotations.data: (E, 4).
    Returns (E, 3) rotation-log residuals.
...
def _so3_hat(phi: torch.Tensor) -> torch.Tensor:
    """Hat operator for SO(3): phi is (..., 3), returns (..., 3, 3)."""
...
def _apply_rotation_delta(
    ...
    """Apply rotation increments: rotations.data is (N, 4), delta is (A, 3)."""
    if delta.numel() == 0:
        return rotations
...
def _apply_translation_scale_delta(
    ...
    poses.data: (N, 7), log_s: (N,).
    translation_delta: (A, 3) for non-anchor nodes.
    scale_delta: (N,) dlog_s for all nodes, including the anchor.
    """
